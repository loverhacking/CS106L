...
HashMapIterator<Map, IsConst>& HashMapIterator<Map, IsConst>::operator++() {
    _node = _node->next;
    if (_node == nullptr) {
        // the current position in the bucket is empty, move to the next bucket which is not empty
        for (++_bucket_idx; _bucket_idx < _buckets_array->size(); ++_bucket_idx) {
            _node = (*_buckets_array)[_bucket_idx];
            if (_node != nullptr) {
                return *this;
            }
        }
