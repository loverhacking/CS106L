...
public:
    ...
    */
    ...
    /*
    * Try to emplace a key-value pair in the map
    * If the key already exists, the function does nothing and returns the existing iterator and false
    * If the key doesn't exist, constructs the mapped value in-place using perfect forwarding and returns the new iterator and true
    *
    * Usage:
    ...
    */
    struct Node
    {
        ...
        */
        ...
        /*
        * Constructor that supports in-place construction of the value_type
        * using piecewise_construct for perfect forwarding
        */
        template<typename... Args>
        Node(std::piecewise_construct_t, std::tuple<const K&> key_tuple, Args&&... args)
            : value(std::piecewise_construct, key_tuple, std::forward<Args>(args)...), next(nullptr) {}
    };
