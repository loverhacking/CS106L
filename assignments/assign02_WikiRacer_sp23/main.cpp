...
int numCommonLinks(const unordered_set<string>& curr_set, const unordered_set<string>& target_set) {
    ...
    return std::count_if(curr_set.begin(), curr_set.end(), [&target_set](const string& elem) {
        return target_set.count(elem) > 0;
    });
}
...
vector<string> findWikiLadder(const string& start_page, const string& end_page) {
    ...
    auto cmp_fn = [&w, &target_set](const vector<string>& left, const vector<string>& right) {
        // Use a static cache to store the common link count that has already been calculated
        static unordered_map<string, int> common_links_cache;

        const string& left_end = left.back();
        const string& right_end = right.back();

        if (common_links_cache.find(left_end) == common_links_cache.end()) {
            common_links_cache[left_end] = numCommonLinks(w.getLinkSet(left_end), target_set);
        }

        if (common_links_cache.find(right_end) == common_links_cache.end()) {
            common_links_cache[right_end] = numCommonLinks(w.getLinkSet(right_end), target_set);
        }

        return common_links_cache[left_end] < common_links_cache[right_end];
    };
    // END STUDENT CODE HERE
    ///////////////////////////////////////////////////////////////////////////////////////////////////
